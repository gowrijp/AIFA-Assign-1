...
def run_a_star_cp(maze, layout, start, end, put_on_a_show, constraints):
    queue = []
    closed = []
    prev = {}
    distance = {}
    ...
    while queue:
        ...
        if open_node == end:
            path = helper_func.reconstruct_path(layout, prev, start, end, waits)
            return path

        if constraints:
            current_constraint = helper_func.import_current_constraints(constraints, timestep)
        ...
        for neighbour in neighbours:
            if neighbour not in closed:
                distance_to_node = distance[open_node] + \
                    helper_func.get_manhattan_distance(open_node, neighbour)
                distance_to_end = helper_func.get_manhattan_distance(neighbour, end)

                if constraints:
                    occupied = helper_func.is_occupied(neighbour, current_constraint)
                else:
                    occupied = False
